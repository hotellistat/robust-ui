...
const onDragStart = function (this: HTMLElement, e: any, vnode: any): void {
  ...
  draggingElementHandle =
    config.handleClass &&
    draggingElement.querySelector(`.${config.handleClass}`)
  draggingElementIndex = Array.from(
    draggingElement?.parentElement?.children as HTMLCollection
...
const makeDraggable = (element: any, vnode: any) => {
  const handleElement =
    config.handleClass && element.querySelector(`.${config.handleClass}`)

  if (handleElement) {
    // handle makes item draggable
    ...
    handleElement.onmouseup = (e: any) => {
      // `dragstart` not triggered onClick, so need this to cover onClick event
...
export default {
  mounted: (listElement: any, binding: any, vnode: any) => {
    ...
    config = {
      ...
      placeholderClass:
        (binding.value && binding.value.placeholderClass) ||
        'v-draggable__placeholder',
    }

    Array.from(listElement.children).forEach((item) =>
      makeDraggable(item, vnode)
    )
