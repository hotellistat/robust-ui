...
<template>
  <div ref="anchorRef">
    <RobustInputWrapper v-if="searchable" :title="title" :hint="hint"
      ...
      <input v-show="open" ref="refSelectInput" v-model="search" size="1"
    ...
    focus-within:ring
    focus-within:dark:border-primary-500" @click="openDropdown">
      <div
        class="flex h-full w-full items-center bg-transparent text-current outline-none"
        :class="[$slots.prefix || condensed ? 'pl-2' : 'pl-3']">
        <div class="min-w-0 select-none truncate">{{ activeItem ?
  ...
  <RobustPopper v-if="anchorRef" v-model:open="open" ref="popperRef"
    class="z-[100] origin-top" :append-to="anchorRef"
    :modifiers="popperModifiers" :options="{
    ...
    <ul v-if="computedOptions.length > 0" class="max-h-72 overflow-auto">
      <li v-for="option in computedOptions" :key="option.value"
        ...
        <PhCheck v-if="modelValue === option.value"
          class="ml-auto text-gray-400" weight="bold" size="14" />
      </li>
    </ul>
