  ...
  <div class="flex">
    ...
    >
      ...
      <template v-for="(value, index) in listPages" :key="index">
        <span
          @click="() => setPage(value)"
          class="paginate-btn relative inline-flex items-center justify-center w-12 py-2 border border-gray-700 bg-white text-sm font-medium text-gray-700 hover:bg-gray-300"
    ...
    <div v-if="showPageSize">
      <div class="flex flex-col ml-2 h-full relative" ref="containerRef">
        <div
          class="flex items-center p-1.5 h-full cursor-pointer border border-gray-700 rounded-md overflow-hidden shadow-inner flex focus-within:outline-none focus-within:ring shadow-outline h-8"
        ...
        <ul
          v-if="isOpen"
          class="absolute top-10 ml-0.5 z-50 shadow-2xl rounded-lg py-2 w-full dark:bg-gray-700 bg-white dark:text-white text-gray-700"
        >
          <li
            v-for="size in [10, 25, 50]"
            class="px-4 py-1.5 cursor-pointer hover:bg-gray-500 transition-colors duration-200 flex justify-center items-center hover:text-white"
            :key="size"
