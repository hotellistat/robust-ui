...
<template>
  <RobustInputWrapper :title="title" :hint="hint" :error="error"
    :class="$props.class" :readonly="readonly" :disabled="disabled"
    :condensed="condensed" v-slot="slotProps">
    <div v-if="$slots.prefix"
      class="flex h-full select-none items-center pr-2 text-gray-400"
      :class="[condensed ? 'pl-2' : 'pl-3']">
      <slot tag="div" name="prefix" />
    </div>
    <input :id="slotProps.cuid" ref="inputRef" v-bind="$attrs"
      v-model="inputFieldValue" :disabled="disabled" size="8"
